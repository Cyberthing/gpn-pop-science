import fragmentShaderAlphaSrc from './lib/shaders_100/fragmentShaderAlpha'
import fragmentShaderPaintSrc from './lib/shaders_100/fragmentShaderPaint'
import vertexShaderSrc from './lib/shaders_100/vertexShader'
...
function setUpShaders () {
  // trace('setUpShaders', this._keys)
  const gl = this._gl
  let keyFunctions = ''

  this._keys.forEach(k => {
    const colorMode = k.colorMode||'RGB'
    const mode = k.mode||'HSV'
    ...
    const edge = isNaN(k.edge) ? 1.0 : k.edge

    // keyFunctions += `pixel.a = distAlpha(${color}, ${tolerance.toFixed(3)}, ${edge.toFixed(3)});\n`

    keyFunctions += `gl_FragColor.rgb = color${colorMode}(pixel.rgb);`;
    keyFunctions += `gl_FragColor.a = distAlpha${mode}(pixel.rgb, ${color}, ${tolerance.toFixed(3)}, ${edge.toFixed(3)});\n`
    if (k.debug) keyFunctions += 'debug();\n'
  })

  if(!keyFunctions)
    keyFunctions += `gl_FragColor = pixel;`

  // trace('keyFunctions', keyFunctions)
...
class ChromaGL {
  ...
  target (target) {
    if (target instanceof HTMLCanvasElement) {
      this._gl = target.getContext('webgl')
  ...
  paint () {
    ...
    // trace('gl.canvas.width, gl.canvas.height', gl.canvas.width, gl.canvas.height)
